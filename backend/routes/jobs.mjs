...
import { createTrainingDeployment, deleteTrainingDeployment, getDeploymentLogs, listDeployments, deploymentInfo} from '../kubernetes/client.mjs';
...
// Get logs from a training job
router.get('/:deploymentName/info', async (req, res) => {
    try {
        // Extracts: deploymentName from req.params
        // Calls: k8sClient.readDeployment()
        const deploymentName = req.params.deploymentName; 
        // gets namespace from url query parameter. 
        const namespace = req.query.namespace || 'default';

        console.log(`Getting info for deployment: ${deploymentName} in namespcae: ${namespace}`);

        const DeploymentInfo = await deploymentInfo(deploymentName, namespace); 
        res.status(200).json({ deployment: deploymentInfo });
        console.log("Deployment info:", JSON.stringify(deploymentInfo, null, 2));
        // Returns: 200 OK with logs on success, 500 Internal Server Error on failure
    } catch (error) {
        console.error("Error getting deployment info:", error);
        res.status(500).json({ message: "Failed to get deployment info", error: error.message });
    }
})
